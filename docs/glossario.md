...
|-------|-------------|-------------------------|
| **Personagem** | Entidade de jogo: nome, conceito, sistema, etc. Pode ser **PC** (atrelado a um jogador) ou **NPC** (atrelado ao mestre). Se o jogador está logado e com o personagem selecionado, joga com ele; senão, o mestre pode tratá-lo como NPC. | `character`, `Character`, `GMCharacter`, Figurinos, elenco, cena |
| **PC** | Personagem de jogador (Player Character). Atrelado a um jogador (`owner_user_id`). | `kind='PC'`, personagens do jogador |
| **NPC** | Personagem do mestre (Non-Player Character). Atrelado ao mestre (`created_by_gm_id`). O mestre pode interpretar no palco um personagem que seria PC se o jogador não estiver com ele selecionado. | `kind='NPC'`, personagens do mestre |
| **Avatar** | Imagem do personagem — a figura exibida na UI (lobby, palco, seleção, etc.). | `default_image_url`, `character_image_url`, `imageUrl`, retrato/portrait |
| **Avatar default** | Imagem principal do personagem (slot 0). Por enquanto é fixa; no futuro haverá feature para trocar o avatar dinamicamente. | `character_image` slot 0, `default_image_url` / `default_image_rev` |
| **Jogador** | Usuário que participa da sessão como jogador (não é o mestre). Pode ter um ou mais personagens; escolhe um para a sessão. Até seis jogadores por sessão. Nome do jogador: por enquanto pode ser o e-mail. | `role === "PLAYER"`, fluxo do jogador, `LobbyScreen`, seleção de personagem |
...
| **Elenco** | Coletivo de personagens (da história ou da cena). Conjunto de personagens vinculados a uma história ou a uma cena. | `scene_character`, `story_character`, `SceneCharactersModal`, personagens da cena |
...
| **Lobby** | Sala de espera onde estão os jogadores logados (e o mestre). Cada jogador aparece com personagem selecionado ou com indicação “(se arrumando)” até escolher um personagem. Áudio compartilhado (LiveKit), escolha de personagem. | `LobbyScreen`, `/api/lobby`, sala LiveKit "lobby" |
